...
to setup
  ca
  crt num_reds
  [set size 3 set heading 90 set color red  set xcor -15	set ycor (who * (33 / num_Reds) - 16 + (33 / num_reds) / 2)]

  crt num_blues
  [set size 3 set heading 270 set color blue  set xcor 15  set ycor (who * (33 / num_Blues) - 16	+ (33 / num_blues) / 2)]
...
; cause the turtles to wiggle around, creating bombs that are patches at a rate specified by the user
; the bombs are colored red or blue, and affect only the opposing team's players
to GO
  ask turtles with [color = red] [bomb RateRed red]
  ask turtles with [color = blue] [bomb RateBlue blue]
   every .1
  [rt random 60
    lt random 60
    fd 0.8
...
; creates bombs for each team at a rate (percentage) specified by the user
; (0% means turtles will create no bombs, whereas 100% means the turtles will create bombs all the time
to bomb [rate colour]
  if random 100 < rate
    [set pcolor colour]
end
